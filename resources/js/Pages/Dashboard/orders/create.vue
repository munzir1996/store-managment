<template>
    <layout>
        ...
        <div class="mt-8">
            ...
            <base-panel class=" mt-4">
            <div class="flex">
                    <button v-for="category in categories" :key="category.id" class="mx-1 bg-gray-800 text-xs text-white px-2 py-1 font-semibold rounded uppercase hover:bg-gray-700">
                        {{category.name}}
                    </button>
                </div>
                <div class="flex">
                    <button v-for="sub_category in sub_categories" :key="sub_category.id" class="mx-1 my-1 bg-gray-800 text-xs text-white px-2 py-1 font-semibold rounded uppercase hover:bg-gray-700">
                        {{sub_category.name}}
                    </button>
                </div>
                <!-- <form @submit.prevent="submit"> -->
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                        ...
                        <div v-for="product in products" :key="product.id" class="mt-4 flex flex-col justify-center items-center max-w-sm mx-auto">
                            <div>
                                <img class="bg-gray-300 h-64 w-full rounded-lg shadow-md bg-cover bg-center" :src="product.image">
                            </div>

                            <div class="w-56 md:w-64 bg-white -mt-10 shadow-lg rounded-lg overflow-hidden">
                                <h3 class="py-2 text-center font-bold uppercase tracking-wide text-gray-800">{{product.name}}
                                </h3>
                                <p class="py-2 text-center font-bold uppercase tracking-wide text-gray-800">الوزن : {{product.weight}}</p>
                                <div class="flex items-center justify-between py-2 px-3 bg-gray-200">
                                    ...
                                    <div>
                                        <button @click="addToCart(product.id)" class=" bg-gray-800 text-xs text-white px-2 py-1 font-semibold rounded uppercase hover:bg-gray-700">
                                            Add to cart
                                        </button>
                                    </div>
                                </div>
